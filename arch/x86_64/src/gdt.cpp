...
int x86_64::gdt::register_ldt(qword base_address , dword limit , byte type , byte flags) {
    ...
    ldt_entry->base_high = base_address >> 24;
    ldt_entry->limit_low = limit & 0xFFFF;
    ldt_entry->limit_high = limit >> 16;
    ldt_entry->flags_low = flags & 0x0F;
    ldt_entry->flags_high = flags >> 4;
    ldt_entry->type = type & 0x0F;
    
    // double the size
    gdt_container->current_index += 3;
    return index;
}